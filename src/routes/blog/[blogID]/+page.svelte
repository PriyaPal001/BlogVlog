...
// @ts-nocheck
  ...
  $: console.log(RouteID);
  $: fufiD = $fufi.filter((item) => item.route == RouteID ) ;
  $: fufiData = fufiD[0];
  $: console.log(fufiData[0]);
</script>
...
>
  ...
  <div class="relative flex items-start justify-between p-4 sm:p-6 lg:p-8">
    <div class="sm:pt-18 pt-44 text-white lg:pt-72">
      <h3 class="text-xl font-bold sm:text-2xl ">{fufiData.first_name + " " +fufiData.last_name}</h3>

      <p class="text-sm">{fufiData.created_at}</p>
    </div>

    <div class="flex ">
      <a href={fufiData.insta_url} target="_blank" class="text-gray-900 mx-1 transition duration-100 hover:text-gray-900 active:text-gray-600">
      ...
      <a href={fufiData.linkedin_url} target="_blank" class="text-gray-900 mx-1 transition duration-100 hover:text-gray-900 active:text-gray-600">
        <svg class="h-5 w-5" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
